...
- Manual checks:
  restarted Butler outside sandbox with `venv/bin/python -u butler.py --clap-only`; the host detected `MacBook Pro Microphone` and armed double-clap listening
  process check showed one Butler owner on `3335` and one dashboard process on `7532/7533`

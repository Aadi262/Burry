...
let operator = bootstrap.operator || {};
...
class BurryOrb {
  ...
  _nodePosition(meta, index, seconds, collapseFactor) {
...
const background = new NeuralBackground(refs.networkCanvas);
const orb = new BurryOrb(refs.orbCanvas);

function setButlerState(mode, note) {
  const cleanedMode = ["idle", "listening", "thinking", "speaking"].includes(mode) ? mode : "idle";
  refs.body.dataset.state = cleanedMode;
  refs.statePillLabel.textContent = cleanedMode.toUpperCase();
  refs.statePillNote.textContent = note || pillNote(operator, cleanedMode);
  orb.setState(cleanedMode);
}

function topSummary(data) {
  if (focusKind === "mood" && data.mood_note) return data.mood_note;
  if (focusKind === "session") {
    return data.session_active
      ? `Session is live. Last update at ${formatClock(data.updated_at)}.`
      : "Session is on standby. Clap or trigger Burry to wake it.";
  }
  const bits = [
    data.focus_project ? `Focus project: ${data.focus_project}` : "",
    data.frontmost_app ? `Frontmost app: ${data.frontmost_app}` : "",
    data.browser_url ? `Browser: ${data.browser_url}` : "",
  ].filter(Boolean);
  return bits.join(" · ") || "Burry is watching your workspace and waiting for the next move.";
}
...
function telemetryEntries(data) {
  const entries = [];
...
function renderTranscript(data) {
  ...
  optimisticEntries = optimisticEntries.filter((entry) => !(entry.role === "user" && seenUserLines.has(entry.text)));
  const entries = [...liveEntries, ...optimisticEntries].slice(-20);
  if (!entries.length) {
...
function renderOperator(data) {
  operator = data || {};
  const mode = normalizeMode(operator);
  refs.modeMood.textContent = operator.mood_label || "Focused";
  refs.modeSession.textContent = operator.session_label || "Standby";
  refs.modeState.textContent = operator.state_label || mode;
...
function handleOperatorTransportMessage(raw) {
  try {
    ...
    if (parsed && parsed.type === "operator" && parsed.payload) {
      renderOperator(parsed.payload);
      return;
    }
    renderOperator(parsed && parsed.payload ? parsed.payload : parsed);
  } catch (error) {
    console.error(error);
  }
}
...
async function refreshOperator() {
  try {
    ...
    renderOperator(await response.json());
  } catch (error) {
    console.error(error);
  }
}
...
function setupSpeechRecognition() {
  ...
  engine.onend = () => {
    ...
    stopMicMonitor();
    setButlerState(normalizeMode(operator), pillNote(operator, normalizeMode(operator)));
  };
...
window.setButlerState = setButlerState;
...
if (!connectOperatorStream()) {
  window.setInterval(refreshOperator, 2000);
}
window.setInterval(refreshProjects, 8000);
